use crate::{helixc::generator::utils::{GeneratedValue, Separator}, protocol::value::casting::CastType};
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        for item in self.body.iter() {
            ...
            writeln!(
                ...
                item.from_item.to_ascii_lowercase(), self.from_version, self.to_version
            )?;
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            GeneratedMigrationPropertyMapping::FieldAdditionFromValue { new_field, value } => {
                write!(
                    f,
                    "field_addition_from_value!(&mut props, &mut new_props, {}, {})",
                    new_field, value
                )
            }
            GeneratedMigrationPropertyMapping::FieldTypeCast { field, cast } => {
                write!(f, "field_type_cast!(&mut props, &mut new_props, {}, {})", field, cast)
            }
        }
    }
}
