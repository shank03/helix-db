...
#[handler]
pub fn inesrtUser (input: &HandlerInput, response: &mut Response) -> Result<(), GraphError> {
let data: inesrtUserInput = match sonic_rs::from_slice(&input.request.body) {
    Ok(data) => data,
    Err(err) => return Err(GraphError::from(err)),
};
...
.add_n("User", Some(props! { "user_num" => data.in_user_num.clone(), "gender" => data.in_gender.clone(), "r_status" => data.in_r_status.clone(), "age" => data.in_age.clone(), "numoffriends" => data.in_numoffriends.clone() }), Some(&["user_num"])).collect_to::<Vec<_>>();
let mut return_vals: HashMap<String, ReturnValue> = HashMap::new();
        return_vals.insert("n".to_string(), ReturnValue::from_traversal_value_array_with_mixin(n.clone(), remapping_vals.borrow_mut()));
