...
#[handler(with_write)]
pub fn createLinkedinEducation(
    input: &HandlerInput,
    response: &mut Response,
) -> Result<(), GraphError> {
    {
        let linkedin_education = G::new_mut(Arc::clone(&db), &mut txn)
.add_n("LinkedinEducation", Some(props! { "location" => &data.location, "school" => &data.school, "field" => &data.field, "date_end" => &data.date_end, "title" => &data.title, "date_start" => &data.date_start, "description" => &data.description }), None).collect_to_obj();
...
#[handler(with_write)]
pub fn updateLinkedinExperience(
    input: &HandlerInput,
    response: &mut Response,
) -> Result<(), GraphError> {
    {
...
#[handler(with_read)]
pub fn getUserLinkedinEducations(
    input: &HandlerInput,
    response: &mut Response,
) -> Result<(), GraphError> {
    {
...
pub fn createWarmConnect(input: &HandlerInput, response: &mut Response) -> Result<(), GraphError> {
    {
        let warm_connect = G::new_mut(Arc::clone(&db), &mut txn)
            .add_n(
                "WarmConnect",
                Some(props! { "name" => &data.name, "email" => &data.email }),
                None,
            )
            .collect_to_obj();
        let user = G::new(Arc::clone(&db), &txn)
...
#[handler(with_read)]
pub fn getUserLinkedinCompanies(
    input: &HandlerInput,
    response: &mut Response,
) -> Result<(), GraphError> {
    {
...
#[handler(with_write)]
pub fn createLinkedinContent(
    input: &HandlerInput,
    response: &mut Response,
) -> Result<(), GraphError> {
    {
        let linkedin_content = G::new_mut(Arc::clone(&db), &mut txn)
.add_n("LinkedinContent", Some(props! { "data_source" => &data.data_source, "country_code" => &data.country_code, "profile_picture" => &data.profile_picture, "linkedin_url" => &data.linkedin_url, "current_position" => &data.current_position, "first_name" => &data.first_name, "connection_count" => &data.connection_count, "name" => &data.name, "full_name" => &data.full_name, "last_name" => &data.last_name, "certifications" => &data.certifications, "languages" => &data.languages, "follower_count" => &data.follower_count, "background_picture" => &data.background_picture, "industry" => &data.industry, "position_end_date" => &data.position_end_date, "country" => &data.country, "extracted_at" => &data.extracted_at, "email" => &data.email, "region" => &data.region, "skills" => &data.skills, "summary" => &data.summary, "position_start_date" => &data.position_start_date, "public_id" => &data.public_id }), None).collect_to_obj();
        let user = G::new(Arc::clone(&db), &txn)
            .n_from_id(&data.user_id)
            .collect_to_obj();
        let metadata = G::new_from(Arc::clone(&db), &txn, user.clone())
...
#[handler(with_write)]
pub fn createMetadataNotes(
    input: &HandlerInput,
    response: &mut Response,
) -> Result<(), GraphError> {
    {
        let user_metadata_notes = G::new_mut(Arc::clone(&db), &mut txn)
.add_n("MetadataNotes", Some(props! { "text" => &data.text, "userScore" => &data.userScore, "aiScore" => &data.aiScore, "flagged" => &data.flagged }), None).collect_to_obj();
        let user = G::new(Arc::clone(&db), &txn)
            .n_from_id(&data.user_id)
            .collect_to_obj();
        let metadata = G::new_from(Arc::clone(&db), &txn, user.clone())
...
#[handler(with_write)]
pub fn createLinkedinExperience(
    input: &HandlerInput,
    response: &mut Response,
) -> Result<(), GraphError> {
    {
        let linkedin_experience = G::new_mut(Arc::clone(&db), &mut txn)
.add_n("LinkedinExperience", Some(props! { "description" => &data.description, "title" => &data.title, "date_end" => &data.date_end, "date_start" => &data.date_start, "field" => &data.field, "company" => &data.company, "location" => &data.location }), None).collect_to_obj();
        let user = G::new(Arc::clone(&db), &txn)
            .n_from_id(&data.user_id)
            .collect_to_obj();
        let metadata = G::new_from(Arc::clone(&db), &txn, user.clone())
...
.add_n("User", Some(props! { "email" => &data.email, "phone" => &data.phone, "elo" => &data.elo, "color" => &data.color, "sender" => &data.sender, "age" => &data.age, "bio" => &data.bio, "location" => &data.location, "profilePic" => &data.profilePic }), None).collect_to_obj();
        let mut return_vals: HashMap<String, ReturnValue> = HashMap::new();
        return_vals.insert(
            "user".to_string(),
            ReturnValue::from_traversal_value_with_mixin(
...
pub fn createName(input: &HandlerInput, response: &mut Response) -> Result<(), GraphError> {
    {
        let name = G::new_mut(Arc::clone(&db), &mut txn)
            .add_n(
                "Name",
                Some(props! { "last" => &data.last, "first" => &data.first }),
                None,
            )
            .collect_to_obj();
        let user = G::new(Arc::clone(&db), &txn)
...
#[handler(with_read)]
pub fn getUserLinkedinExperiences(
    input: &HandlerInput,
    response: &mut Response,
) -> Result<(), GraphError> {
    {
...
pub fn searchUsersByBio(input: &HandlerInput, response: &mut Response) -> Result<(), GraphError> {
    {
        let similar_bios = G::new(Arc::clone(&db), &txn)
            .search_v::<fn(&HVector, &RoTxn) -> bool>(&data.bio_vector, data.k as usize, None)
            .collect_to::<Vec<_>>();
...
#[handler(with_read)]
pub fn getUserLinkedinWebsites(
    input: &HandlerInput,
    response: &mut Response,
) -> Result<(), GraphError> {
    {
...
#[handler(with_read)]
pub fn getUserLinkedinCurrentCompany(
    input: &HandlerInput,
    response: &mut Response,
) -> Result<(), GraphError> {
    {
...
#[handler(with_write)]
pub fn deleteLinkedinEducation(
    input: &HandlerInput,
    response: &mut Response,
) -> Result<(), GraphError> {
    {
...
.add_n("Metadata", Some(props! { "last_updated_ts" => &data.last_updated_ts, "referredBy" => &data.referredBy, "archetype" => &data.archetype, "created_ts" => &data.created_ts }), None).collect_to_obj();
        let user = G::new(Arc::clone(&db), &txn)
            .n_from_id(&data.user_id)
            .collect_to_obj();
...
#[handler(with_write)]
pub fn deleteLinkedinExperience(
    input: &HandlerInput,
    response: &mut Response,
) -> Result<(), GraphError> {
    {
...
#[handler(with_write)]
pub fn updateLinkedinEducation(
    input: &HandlerInput,
    response: &mut Response,
) -> Result<(), GraphError> {
    {
...
#[handler(with_write)]
pub fn createLinkedinCompany(
    input: &HandlerInput,
    response: &mut Response,
) -> Result<(), GraphError> {
    {
        let linkedin_company = G::new_mut(Arc::clone(&db), &mut txn)
.add_n("LinkedinCompany", Some(props! { "name" => &data.name, "specialties" => &data.specialties, "industry" => &data.industry, "website" => &data.website, "founded" => &data.founded, "domain" => &data.domain, "headquarters" => &data.headquarters, "staff_count" => &data.staff_count, "description" => &data.description }), None).collect_to_obj();
        let user = G::new(Arc::clone(&db), &txn)
            .n_from_id(&data.user_id)
            .collect_to_obj();
        let metadata = G::new_from(Arc::clone(&db), &txn, user.clone())
