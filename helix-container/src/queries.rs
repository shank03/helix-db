...
.add_n("Doctor", Some(props! { "city" => data.doctor_city.clone(), "name" => data.doctor_name.clone() }), None).collect_to::<Vec<_>>();
...
.add_e("Visit", Some(props! { "doctors_summary" => data.summary.clone(), "date" => data.date.clone() }), patient.id(), doctor.id(), true, EdgeType::Node).collect_to::<Vec<_>>();
let mut return_vals: HashMap<String, ReturnValue> = HashMap::new();
        return_vals.insert("patient".to_string(), ReturnValue::from_traversal_value_array_with_mixin(patient.clone(), remapping_vals.borrow_mut()));
