...
.add_n("User", Some(props! { "name" => &data.name, "email" => &data.email, "gh_login" => &data.gh_login, "gh_id" => &data.gh_id, "created_at" => chrono::Utc::now().to_rfc3339(), "updated_at" => chrono::Utc::now().to_rfc3339() }), Some(&["gh_id"])).collect_to_obj();
        let mut return_vals: HashMap<String, ReturnValue> = HashMap::new();
        return_vals.insert(
            "user".to_string(),
            ReturnValue::from_traversal_value_with_mixin(
...
.add_n("Cluster", Some(props! { "region" => &data.region, "api_url" => "", "created_at" => chrono::Utc::now().to_rfc3339(), "updated_at" => chrono::Utc::now().to_rfc3339(), "status" => "pending" }), None).collect_to_obj();
        let new_instance = G::new_mut(Arc::clone(&db), &mut txn)
.add_n("Instance", Some(props! { "storage_gb" => &data.storage_gb, "ram_gb" => &data.ram_gb, "created_at" => chrono::Utc::now().to_rfc3339(), "updated_at" => chrono::Utc::now().to_rfc3339(), "instance_type" => &data.instance_type, "region" => &data.region, "status" => "pending", "api_url" => "" }), None).collect_to_obj();
