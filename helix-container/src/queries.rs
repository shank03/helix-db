...
.add_n("User", Some(props! { "profilePic" => &data.profilePic, "bio" => &data.bio, "phone" => &data.phone, "sender" => &data.sender, "location" => &data.location, "color" => &data.color, "email" => &data.email, "age" => &data.age, "elo" => &data.elo }), None).collect_to_obj();
...
.add_n("LinkedinExperience", Some(props! { "title" => &data.title, "company" => &data.company, "date_start" => &data.date_start, "description" => &data.description, "location" => &data.location, "field" => &data.field, "date_end" => &data.date_end }), None).collect_to_obj();
...
.add_n("LinkedinWebsite", Some(props! { "url" => &data.url, "category" => &data.category }), None).collect_to_obj();
    let user = G::new(Arc::clone(&db), &txn)
.n_from_id(&data.user_id).collect_to_obj();
    let metadata = G::new_from(Arc::clone(&db), &txn, user.clone())
...
.add_n("WarmConnect", Some(props! { "name" => &data.name, "email" => &data.email }), None).collect_to_obj();
    let user = G::new(Arc::clone(&db), &txn)
.n_from_id(&data.user_id).collect_to_obj();
    let metadata = G::new_from(Arc::clone(&db), &txn, user.clone())
...
.add_n("LinkedinContent", Some(props! { "public_id" => &data.public_id, "languages" => &data.languages, "country" => &data.country, "position_end_date" => &data.position_end_date, "data_source" => &data.data_source, "linkedin_url" => &data.linkedin_url, "background_picture" => &data.background_picture, "connection_count" => &data.connection_count, "certifications" => &data.certifications, "extracted_at" => &data.extracted_at, "first_name" => &data.first_name, "email" => &data.email, "profile_picture" => &data.profile_picture, "summary" => &data.summary, "full_name" => &data.full_name, "current_position" => &data.current_position, "name" => &data.name, "region" => &data.region, "country_code" => &data.country_code, "skills" => &data.skills, "industry" => &data.industry, "follower_count" => &data.follower_count, "last_name" => &data.last_name, "position_start_date" => &data.position_start_date }), None).collect_to_obj();
    let user = G::new(Arc::clone(&db), &txn)
.n_from_id(&data.user_id).collect_to_obj();
    let metadata = G::new_from(Arc::clone(&db), &txn, user.clone())
...
.add_n("Metadata", Some(props! { "referredBy" => &data.referredBy, "created_ts" => &data.created_ts, "archetype" => &data.archetype, "last_updated_ts" => &data.last_updated_ts }), None).collect_to_obj();
    let user = G::new(Arc::clone(&db), &txn)
.n_from_id(&data.user_id).collect_to_obj();
...
.add_n("LinkedinEducation", Some(props! { "date_start" => &data.date_start, "description" => &data.description, "title" => &data.title, "field" => &data.field, "school" => &data.school, "date_end" => &data.date_end, "location" => &data.location }), None).collect_to_obj();
    let user = G::new(Arc::clone(&db), &txn)
.n_from_id(&data.user_id).collect_to_obj();
    let metadata = G::new_from(Arc::clone(&db), &txn, user.clone())
...
.add_n("Name", Some(props! { "first" => &data.first, "last" => &data.last }), None).collect_to_obj();
    let user = G::new(Arc::clone(&db), &txn)
.n_from_id(&data.user_id).collect_to_obj();
...
.add_n("MetadataNotes", Some(props! { "flagged" => &data.flagged, "text" => &data.text, "userScore" => &data.userScore, "aiScore" => &data.aiScore }), None).collect_to_obj();
    let user = G::new(Arc::clone(&db), &txn)
.n_from_id(&data.user_id).collect_to_obj();
    let metadata = G::new_from(Arc::clone(&db), &txn, user.clone())
...
.add_n("LinkedinCompany", Some(props! { "staff_count" => &data.staff_count, "website" => &data.website, "industry" => &data.industry, "headquarters" => &data.headquarters, "description" => &data.description, "founded" => &data.founded, "domain" => &data.domain, "specialties" => &data.specialties, "name" => &data.name }), None).collect_to_obj();
    let user = G::new(Arc::clone(&db), &txn)
.n_from_id(&data.user_id).collect_to_obj();
    let metadata = G::new_from(Arc::clone(&db), &txn, user.clone())
