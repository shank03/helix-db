...
#[handler]
pub fn get_patients_visits_in_previous_month(
    input: &HandlerInput,
    response: &mut Response,
) -> Result<(), GraphError> {
    let data: get_patients_visits_in_previous_monthInput =
...
pub fn create_data(input: &HandlerInput, response: &mut Response) -> Result<(), GraphError> {
    ...
    let doctor = G::new_mut(Arc::clone(&db), &mut txn)
        .add_n(
            "Doctor",
            Some(props! { "name" => data.doctor_name.clone(), "city" => data.doctor_city.clone() }),
            None,
        )
        .collect_to::<Vec<_>>();
    let patient = G::new_mut(Arc::clone(&db), &mut txn)
        .add_n(
            "Patient",
            Some(props! { "age" => data.patient_age.clone(), "name" => data.patient_name.clone() }),
            None,
        )
        .collect_to::<Vec<_>>();
    G::new_mut(Arc::clone(&db), &mut txn)
        .add_e(
            "Visit",
            Some(props! { "doctors_summary" => data.summary.clone(), "date" => data.date.clone() }),
            patient.id(),
            doctor.id(),
            true,
            EdgeType::Node,
