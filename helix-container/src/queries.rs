...
pub struct create_preferencesInput {
    pub preferences: Vec<String>,
}
#[handler]
pub fn create_preferences(input: HandlerInput) -> Result<Response, GraphError> {
    let db = Arc::clone(&input.graph.storage);
    let data = input
        .request
        .in_fmt
        .deserialize::<create_preferencesInput>(&input.request.body)?
        .into_owned();
    Err(IoContFn::create_err(
        move |__internal_cont_tx, __internal_ret_chan| {
            Box::pin(async move {
                let __internal_embed_data_0 = embed_async!(db, &preference);
                __internal_cont_tx
                    .send_async((
                        __internal_ret_chan,
                        Box::new(move || {
                            ...
                            for preference in &data.preferences {
                                G::new_mut(Arc::clone(&db), &mut txn)
                                    .insert_v::<fn(&HVector, &RoTxn) -> bool>(
                                        ...
                                        "Preference",
                                        Some(props! { "preference" => preference.clone() }),
                                    )
                                    .collect_to_obj();
                            }
