...
pub fn find_users_exclusion(
    ...
    return_vals.insert(
        "users".to_string(),
        ReturnValue::from_traversal_value_array_with_mixin(
            G::new_from(Arc::clone(&db), &txn, users.clone())
                .map_traversal(|item, txn| {
                    exclude_field!(remapping_vals, item.clone(), "name", "email")?;
                    Ok(item)
                })
                .collect_to::<Vec<_>>()
                .clone(),
...
#[handler]
pub fn find_user_posts_with_creator_details(
    input: &HandlerInput,
    response: &mut Response,
) -> Result<(), GraphError> {
    let data: find_user_posts_with_creator_detailsInput =
...
pub fn create_follow(input: &HandlerInput, response: &mut Response) -> Result<(), GraphError> {
    ...
    let follower = G::new(Arc::clone(&db), &txn)
        .n_from_id(&data.follower_id)
        .collect_to::<Vec<_>>();
    let followed = G::new(Arc::clone(&db), &txn)
        .n_from_id(&data.followed_id)
        .collect_to::<Vec<_>>();
    G::new_mut(Arc::clone(&db), &mut txn)
        .add_e(
            "Follows",
            Some(props! { "since" => data.now.clone() }),
            follower.id(),
            followed.id(),
            true,
            EdgeType::Node,
...
.add_n("Post", Some(props! { "created_at" => data.now.clone(), "content" => data.content.clone(), "updated_at" => data.now.clone() }), None).collect_to::<Vec<_>>();
    G::new_mut(Arc::clone(&db), &mut txn)
        .add_e(
            "Created",
            Some(props! { "created_at" => data.now.clone() }),
            user.id(),
            post.id(),
            true,
            EdgeType::Node,
...
#[handler]
pub fn get_name_remapping_simple(
    input: &HandlerInput,
    response: &mut Response,
) -> Result<(), GraphError> {
    let mut remapping_vals = RemappingMap::new();
